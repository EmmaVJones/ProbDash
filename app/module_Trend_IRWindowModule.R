...
trendIRWindow <- function(input,output,session, parameterData, barData, thresholdValue, IRWindowSubpopulations){
  ns <- session$ns
  ...
  cdfData <- reactive({
    ...
    z$Subpopulation <- factor(z$Subpopulation, levels = IRWindowSubpopulations, ordered = T)
  ...
  output$plot <- renderPlotly({
    ...
    if(input$radio %in% 'Boxplot'){
      ...
      plot_ly(cdfData()) %>%
        ...
        addMoE(cdfData(),"IR2020") %>%
        addMoE(cdfData(),"IR2022") %>%
