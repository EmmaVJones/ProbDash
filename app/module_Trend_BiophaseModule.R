...
trendBiophase <- function(input,output,session, parameterData, barData, thresholdValue, biophaseSubpopulations){
  ns <- session$ns
  
  cdfData <- reactive({
    req(parameterData)
    z <- filter(parameterData(), Subpopulation %in% biophaseSubpopulations)  %>%
      mutate(ymin = Estimate.P - MoE, ymax = Estimate.P + MoE) 
    z$Subpopulation <- factor(z$Subpopulation, levels = biophaseSubpopulations, ordered = T)
  ...
  output$plot <- renderPlotly({
    ...
    if(input$radio %in% 'Boxplot'){
      ...
      plot_ly(cdfData()) %>%
        ...
        addMoE(cdfData(),biophaseSubpopulations[1]) %>% 
        addMoE(cdfData(),biophaseSubpopulations[2]) %>% 
      # addMoE(cdfData(),"Phase One 2001-2008") %>%
      #   addMoE(cdfData(),"Phase Two 2009-2018") %>%
