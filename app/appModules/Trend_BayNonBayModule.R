...
trendBayNonBay <- function(input,output,session, parameterData, barData, thresholdValue){
  ns <- session$ns
  ...
  output$plot <- renderPlotly({
    ...
    if(input$radio %in% 'Boxplot'){
      ...
      plot_ly(cdfData()) %>%
        ...
        addMoE(cdfData(),"Bay Watersheds 2001-2016") %>%
        addMoE(cdfData(),"Non-Bay Watersheds 2001-2016") %>%
        addMoE(cdfData(),"Bay Watersheds 2001-2009") %>%
        addMoE(cdfData(),"Non-Bay Watersheds 2001-2009") %>%
        addMoE(cdfData(),"Bay Watersheds 2009-2016") %>%
        addMoE(cdfData(),"Non-Bay Watersheds 2009-2016") %>%
