...
statusStreamSizeUI <- function(id){
  ns <- NS(id)
  tagList(
    wellPanel(
...
statusStreamSize <- function(input,output,session, parameterData, barData, thresholdValue){
  ns <- session$ns
  
  cdfData <- reactive({
    req(parameterData)
    z <- filter(parameterData(), Subpopulation %in% streamSizeSubpopulations)  %>%
  ...
  output$plot <- renderPlotly({
    ...
    if(input$radio %in% 'Boxplot'){
      ...
      plot_ly(cdfData()) %>%
        ...
        addMoE(cdfData(),"Small") %>%
        addMoE(cdfData(),"Medium") %>%
        addMoE(cdfData(),"Large") %>%
