...
statusStreamOrderUI <- function(id){
  ns <- NS(id)
  tagList(
    wellPanel(
...
statusStreamOrder <- function(input,output,session, parameterData, barData, thresholdValue){
  ns <- session$ns
  
  cdfData <- reactive({
    req(parameterData)
    z <- filter(parameterData(), Subpopulation %in% streamOrderSubpopulations)  %>%
  ...
  output$plot <- renderPlotly({
    ...
    if(input$radio %in% 'Boxplot'){
      ...
      plot_ly(cdfData()) %>%
        ...
        addMoE(cdfData(),'First Order') %>%
        addMoE(cdfData(),'Second Order') %>%
        addMoE(cdfData(),'Third Order') %>%
        addMoE(cdfData(),'Fourth Order') %>%
        addMoE(cdfData(),'Fifth Order') %>%
